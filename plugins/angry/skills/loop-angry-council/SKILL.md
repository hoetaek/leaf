---
name: loop-angry-council
description: "Bounded review/fix loop using angry-council as the engine — convene the panel, fix the chair's top verdict, verify, repeat within budget (default 2). Stops on cross-axis tradeoffs needing a human. Triggers: angry-council loop, loop-angry-council, panel review loop."
---
...
Use `angry-council` as the review engine. Before the first pass, read
`../angry-council/SKILL.md` if available and follow its panel triage, independent
...
- a chair verdict that is a cross-axis **tradeoff** (e.g. `angry-jobs` "cut it"
  vs `angry-ramsay` "not done yet") rather than a true contradiction — that needs
...
1. Fix the scope: what is under review + Context Pack (Known Facts, Fixed
   Constraints), per the `angry-council` protocol.
2. Run one `angry-council` pass: triage the 3-5 relevant personas, have them
